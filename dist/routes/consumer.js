...
router.post('/reserve', authenticate, requireRole(['consumer', 'admin']), body('producerId').isInt({ min: 1 }), body('date').isISO8601(), body('hour').isInt({ min: 0, max: 23 }), body('kwh').isFloat({ min: 0.1 }), async (req, res) => {
...
router.post('/modify', authenticate, requireRole(['consumer', 'admin']), body('reservationId').isInt({ min: 1 }), body('kwh').isFloat({ min: 0 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const consumerId = req.user.sub;
...
router.get('/purchases', authenticate, requireRole(['consumer', 'admin']), query('producerId').optional().isInt({ min: 1 }), query('energyType').optional().isIn(['Fossile', 'Eolico', 'Fotovoltaico']), query('range').optional().isString(), async (req, res) => {
    const consumerId = req.user.sub;
...
router.get('/carbon', authenticate, requireRole(['consumer', 'admin']), query('range').isString(), async (req, res) => {
    const consumerId = req.user.sub;
    const [start, end] = String(req.query.range).split('|');

...
router.post('/profile', authenticate, requireRole(['producer', 'admin']), body('energyType').isIn(['Fossile', 'Eolico', 'Fotovoltaico']), body('co2PerKwh').isFloat({ min: 0 }), body('pricePerKwh').optional().isFloat({ min: 0 }), body('defaultMaxPerHourKwh').isFloat({ min: 0 }), async (req, res) => {
...
router.post('/capacities', authenticate, requireRole(['producer', 'admin']), body('date').isISO8601().toDate(), body('slots').isArray({ min: 1 }), body('slots.*.hour').isInt({ min: 0, max: 23 }), body('slots.*.maxCapacityKwh').isFloat({ min: 0 }), body('slots.*.pricePerKwh').optional().isFloat({ min: 0 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const userId = req.user.sub;
...
router.get('/occupancy', authenticate, requireRole(['producer', 'admin']), query('date').isISO8601(), query('fromHour').optional().isInt({ min: 0, max: 23 }), query('toHour').optional().isInt({ min: 0, max: 23 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const userId = req.user.sub;
...
router.post('/prices', authenticate, requireRole(['producer', 'admin']), body('date').isISO8601(), body('slots').isArray({ min: 1 }), body('slots.*.hour').isInt({ min: 0, max: 23 }), body('slots.*.pricePerKwh').isFloat({ min: 0 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const userId = req.user.sub;
...
router.get('/earnings', authenticate, requireRole(['producer', 'admin']), query('range').isString(), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const userId = req.user.sub;
...
router.post('/proportional-accept', authenticate, requireRole(['producer', 'admin']), body('date').isISO8601(), body('hour').isInt({ min: 0, max: 23 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const userId = req.user.sub;
    ...
    for (const r of reservations) {
        ...
        if (newKwh < 0.1)
            continue; // keep minimum granularity out of scope
